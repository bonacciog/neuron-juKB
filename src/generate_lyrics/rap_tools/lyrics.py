...
class Lyrics:
    ...
    def __init__(self, filename=None, print_stats=False, text=None, 
                 language='fi', lookback=10, precomputed = None):
        '''
        ...
        if ((self.precomputed != None)):
            #print("print precomputed in lyrics: ", self.precomputed)
            stl = list(self.precomputed)
            #print("print stl in lyrics: ", stl)
            stl[-1] = " "
            #print("print stl in lyrics after replacement: ", stl)
            self.precomputed = "".join(stl)
            #print("print precomputed in lyrics after replacement: ", self.precomputed)
        ...
        if self.text_raw is not None:
            ...
            self.RL = self.rhyme_length(len(self.words) - 1)[0]
    ...
    def compute_vowel_representation(self):
        ...
        if len(self.language) >= 2 and self.language[:2] == 'en':
            ...
            if(self.precomputed != None):
                ...
                phon_trans = ph.get_phonetic_transcription(self.text, output_fname=None)
                #print("Phon transcription of text: ", phon_trans)
                #print("Precomputed:" , self.precomputed)
                self.text = self.precomputed + phon_trans
                #print("Total text, precomputed + text: ", self.text)
            else:
                self.text = ph.get_phonetic_transcription(self.text, output_fname=None)
            self.word_ends_orig = []
            self.words_orig = []
